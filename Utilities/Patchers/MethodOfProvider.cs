...
namespace XansTools.Utilities.Patchers {
	public static class MethodOfProvider {
		...
		/// <summary>
		/// The same as <see cref="methodof{T}(T)"/> but explicitly for use in Harmony.
		/// </summary>
		/// <typeparam name="T"></typeparam>
		/// <param name="del"></param>
		/// <returns></returns>
		public static HarmonyMethod HarmonyMethodOf<T>(T del) where T : Delegate {
			return new HarmonyMethod((MethodInfo)methodof(del));
		}
	}
}
